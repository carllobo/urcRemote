...
public class ConnectionHandshakeMethod extends UrcNetworkMethod {
	
	public static final UrcNetworkMethod DEFAULT = new ConnectionHandshakeMethod(false);
	
	private static final byte MY_HEADER = 7;
	
	private boolean fullDuplex;
	
	public ConnectionHandshakeMethod(boolean fullDuplex) {
		super(MY_HEADER);
	...
	protected UrcNetworkMethod readPayload(DataInputStream input)
			throws IOException {
		boolean fullDuplex = input.readBoolean();
		return new ConnectionHandshakeMethod(fullDuplex);
	}

	protected void writePayload(DataOutputStream output) throws IOException {
		output.writeBoolean(fullDuplex);
	}
